...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    numbers = torch.arange(start, stop + 1,dtype=torch.float64)
    x = numbers[numbers % 10 == 0]
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    front = x[:12]
    back = x[12:]
    front = front.view(3, 4)
    back = back.view(3, 4)
    y = torch.cat((front, back),  dim=1)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    y[torch.arange(x.shape[0]), y.argmin(dim=1)] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    # 获取批次大小 B，以及其他维度 N, M, P
    B, N, M = x.shape
    _, _, P = y.shape

    # 初始化结果张量 z，形状为 (B, N, P)，并确保数据类型与 x 相同
    z = torch.zeros(B, N, P, dtype=x.dtype)

    # 循环遍历每个批次
    for i in range(B):
        z[i] = torch.mm(x[i], y[i])  # 对第 i 个批次的矩阵进行乘法
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    count = x.shape[0]
    means = sums / count
    variance = torch.sum((x - means) ** 2, dim=0) / (count - 1)
